...
class Database {
    ...
    private function log_query($q){
        $fh = fopen("db_query.log","at");
        fwrite($fh,$q."\n");
        fclose($fh);
    }

    public function sanitizeFormData($d){

        return $this->db->escape_string($d);
    }
...
$sql = "
SELECT `products`.*,`images`.`imagename`
FROM `products`
